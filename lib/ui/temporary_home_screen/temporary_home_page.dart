...
class TemporaryHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(create: (BuildContext context) => TemporaryHomePageCubit(), child: TemporaryHomePageView());
  }
}
...
class _TemporaryHomePageViewState extends State<TemporaryHomePageView> {
  ...
  void showToaster({required String message, bool isError = false}) {
    Fluttertoast.showToast(
      msg: message,
      toastLength: Toast.LENGTH_SHORT,
      gravity: ToastGravity.BOTTOM,
      timeInSecForIosWeb: 1,
      backgroundColor: isError ? Colors.red : Color(0xFF83D481),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: BlocConsumer<TemporaryHomePageCubit, TemporaryHomePageStates>(
          listenWhen: (previous, current) => current is HomeClickMeActionState || current is HomeActionButtonState,
          buildWhen: (previous, current) => current is FetchHomePageState,
          listener: (context, state) {
            if (state is HomeClickMeActionState && state.message.isNotEmpty) {
              showToaster(message: state.message, isError: state.status == StateStatus.error);
            }
            if (state is HomeActionButtonState && state.message.isNotEmpty) {
              showToaster(message: state.message, isError: state.status == StateStatus.error);
            }
          },
                              ...
                              child: ListView.separated(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  return TextButton(
                                    onPressed: () async {
                                      final cubit = context.read<TemporaryHomePageCubit>();
                                      await cubit.actionButton(id: state.response['instant_actions'][index]['id']);
                                    },
